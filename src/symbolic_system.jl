...
∂t = Differential(t)
∂x = Differential.((x, y, z))

eqs = [
    ∂t(ϕ(t,x,y,z)) ~ - α(t,x,y,z) * K(t,x,y,z),
    ∂t(α(t,x,y,z)) ~ -2 * α(t,x,y,z) * K(t,x,y,z),
    ∂t(K(t,x,y,z)) ~ -∂x[1](∂x[1](ϕ(t,x,y,z))),
]
...
@named bssn_system = PDESystem(eqs, bcs, domains, [t, x, y, z], [ϕ(t,x, y, z), α(t,x, y, z), K(t,x, y, z)])
...
discretization = MOLFiniteDifference([x => dx, y => dy, z => dz], t, approx_order = order, grid_align = center_align)
